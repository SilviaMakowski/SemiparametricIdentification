...
function[train,ini_train,test, ini_test, individuals_types_count]=parse_data(all_data, train_ids,test_ids, types)
    ...
    parfor individual = 1: individual_number %looping over all the individuals in the dataset
        ...
        for train_sen=train_sens' 
            ...
            if(size(train_temp,1)>0)
                ini_train_id{1}=[ini_train_id{1},train_temp(1,all_data.desc(1))];%amplitue
                ini_train_id{2}=[ini_train_id{2},train_temp(1,all_data.desc(2))];%duration
            end
        end
        ...
        for test_sen=test_sens' 
            ...
            if(size(test_data{test_sent_index},1)>0)
                ini_test_id{1}=[ini_test_id{1},test_data{test_sent_index}(1,all_data.desc(1))];%amplitude
                ini_test_id{2}=[ini_test_id{2},test_data{test_sent_index}(1,all_data.desc(2))];%duration
            end
        ...
        types_count=[];
        for type=types 
            types_count(type)=size(train_data(train_data(:,all_data.desc(5))==type,:),1);
            if types_count(type) < datapts_min_no %checking if the number of data points of specific type for specific individual is less that 20 data points
                ...
                train_data(train_data(:,all_data.desc(5))==type,:)=[];
                train_data=[train_data;abs(all_train_data(all_train_data(:,all_data.desc(5))==type,:))];
            end
